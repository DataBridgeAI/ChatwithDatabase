...
# def check_query_relevance(user_input: str, threshold: float = 0.70) -> Dict[str, Any]:
    """
    Check if the user query is relevant to the schema with schema-agnostic
    ...
    Returns:
        Dict with:
            - relevant: bool - Is the query relevant?
            - score: float - Highest relevance score
            - matches: List - Top matching schema elements
            - confidence: str - Low/Medium/High confidence
    """
    try:
        ...
        if not schema_embeddings:
            ...
            return {
                "relevant": True,  # Default to true in case of errors
                "score": 0.0,
                "confidence": "Error",
                "matches": [],
                "error": "Could not load schema embeddings"
            }
        
        schema_keys = list(schema_embeddings.keys())
        
        # Step 3: Extract potential schema terms from query
        schema_terms = extract_potential_schema_terms(user_input)
        
        # Step 4: Find fuzzy matches based on extracted terms
        ...
        # Step 6: Query the vector database for semantic matches
        ...
        # Step 7: Extract semantic results
        semantic_matches = []
        ...
        # Step 8: Combine matches and calculate relevance score
        all_matches = []
        ...
        for match in fuzzy_matches:
            all_matches.append({
                "schema_element": match["schema_element"],
                "score": match["match_score"] * 0.9,  # Slightly reduce fuzzy match confidence
                "match_type": "fuzzy",
        ...
        # Step 9: Merge duplicate matches (take the highest score)
        unique_matches = {}
        ...
        # Step 10: Sort matches by score
        top_matches = sorted(list(unique_matches.values()), key=lambda x: x["score"], reverse=True)
        
        # Step 11: Calculate combined relevance score
        # For multi-term queries, if multiple terms match different schema elements, boost the overall score
        combined_score = 0.0
        if top_matches:
            # Primary score from best match
            combined_score = top_matches[0]["score"]
            
            # Boost for multiple term matches (schema diversity)
            matched_terms = set()
            matched_tables = set()
            matched_columns = set()
            
            for match in top_matches[:5]:  # Consider top 5 matches
            ...
            # Apply boost based on the percentage of extracted terms that matched
            if schema_terms and matched_terms:
                term_match_ratio = len(matched_terms) / len(schema_terms)
                term_boost = min(term_match_ratio * 0.1, 0.1)  # Max 0.1 boost
                combined_score = min(combined_score + term_boost, 1.0)
            
            # Apply small boost for schema diversity
        ...
        # Step 12: Determine relevance with confidence level
        ...
        return {
            ...
            "matches": top_matches[:5],  # Return top 5 matches
            "extracted_terms": schema_terms
        }
    ...
    Args:
        user_input: The user's query string
        schema_embeddings: Optional pre-loaded schema embeddings
        threshold: Similarity threshold for relevance
    ...
    try:
        ...
        is_relevant = combined_score >= threshold
        if not is_relevant:
            print(f"❌ Query is NOT RELEVANT to the schema")
