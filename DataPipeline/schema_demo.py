...
CHROMADB_COLLECTION = "bigquery_schemas"

# Use GCS path for persistent ChromaDB storage
CHROMADB_JSON_PATH = f"gs://{BUCKET_NAME}/chroma_embeddings.json"
...
# Initialize ChromaDB (Memory-based, since Composer does not persist files)
        ...
        for table in tables:
            ...
            for field in table_ref.schema:
                ...
                if field.field_type in ["INTEGER", "FLOAT", "NUMERIC", "DATE", "TIMESTAMP"]:
                    ...
                    # Convert any Timestamp to string before adding it to column_data
                    column_data["min_value"] = df["min_value"].iloc[0].isoformat() if not df.empty and isinstance(df["min_value"].iloc[0], pd.Timestamp) else df["min_value"].iloc[0]
                    column_data["max_value"] = df["max_value"].iloc[0].isoformat() if not df.empty and isinstance(df["max_value"].iloc[0], pd.Timestamp) else df["max_value"].iloc[0]
    ...
    for dataset_id, tables in schema_info.items():
        for table_id, schema in tables.items():
            ...
            all_embeddings.append({
                "id": doc_id,
                "embedding": embedding,
                "metadata": {"dataset": dataset_id, "table": table_id}
            })

            print(f"Stored embedding for {doc_id} in ChromaDB")

    # Store embeddings in GCS as JSON
    gcs_filename = "/home/airflow/gcs/data/chroma_embeddings.json"
    with open(gcs_filename, "w") as f:
        json.dump(all_embeddings, f, indent=2, cls=CustomJSONEncoder)

    # Upload to GCS
    bucket = storage_client.bucket(BUCKET_NAME)
    blob = bucket.blob("chroma_embeddings.json")
    blob.upload_from_filename(gcs_filename)
    print("ChromaDB embeddings stored in GCS successfully!")
